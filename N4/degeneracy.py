...
def dendro(params, target_params, typ, dist_thresh):
    ...
    plt.figure()
    d = dendrogram(Z, orientation='left', labels=list(np.arange(params.shape[0]))+['*'], color_threshold=dist_thresh*max(Z[:,2]), get_leaves=True)
...
#    d1, c1, cl1 = dendro(bi1, target_genotype, 'weighted', 0.65)
#    
#    x_embedded = TSNE(n_components=2, perplexity=5).fit_transform(bi)
#    plt.figure()
